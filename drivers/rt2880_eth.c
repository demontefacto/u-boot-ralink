...
#define CONFIG_RTL8211E_SUPPORT	1
#if defined(CONFIG_RTL8211E_SUPPORT)
/* speed led */
#define REALTEK_LED_CFG1_REG	28
#define REALTEK_LED_CFG1 	0x43

/* active  led */
#define REALTEK_LED_CFG2_REG	26
#define REALTEK_LED_CFG2 	0x30

#define EV_REALTEK_PHY_ID0 0x0001C
#define EV_REALTEK_PHY_ID1 0x0C915
static int isRealtekGigaPHY(int ge)
{
	u32 phy_id0,phy_id1;
        u32 phy_addr = 0;
...
#if defined(CONFIG_RTL8211E_SUPPORT)
	if(isRealtekGigaPHY(1)) 
	{
	    printf("GE1:Found Realtek RTL8211E!\n");
	    mii_mgr_read(MAC_TO_GIGAPHY_MODE_ADDR, 0x10, &phy_val);
	    phy_val |= 1<<4; //disable CLK125
	    mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 0x10, phy_val);
	}
#endif
...
{	  
	   //switch reg page
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 31, 0x0007);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 31, 0x0007);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 30, 0x002c);
	  
	  //Disabel PHY0's address 0
	  mii_mgr_read(MAC_TO_GIGAPHY_MODE_ADDR, 27, &phy_val);
	  phy_val &= ~(1<<2);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 27, phy_val);
	  
	  //Config PHY's LEDs
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR,REALTEK_LED_CFG1_REG,REALTEK_LED_CFG1);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR,REALTEK_LED_CFG2_REG,REALTEK_LED_CFG2);
		
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 31, 0x0000);
	  
	   //switch reg page
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 31, 0x0007);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 30, 0x00a0);
	  //enable SSC
	  mii_mgr_read(MAC_TO_GIGAPHY_MODE_ADDR, 26, &phy_val);
	  phy_val &= ~(1<<2);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 26, phy_val);
	 
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR, 31, 0x0000);
}
 RALINK_REG(RT2880_SYSCFG1_REG) &= ~(0x3<<14); ////GE2_MODE=RGMii Mode
if(isRealtekGigaPHY(2)) 
{
	  u32 value;
	  
	  //switch reg page
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 31, 0x0007);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 31, 0x0007);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 30, 0x002c);
	  
	  //Disabel PHY0's address 0
	  mii_mgr_read(MAC_TO_GIGAPHY_MODE_ADDR2, 27, &phy_val);
	  phy_val &= ~(1<<2);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 27, phy_val);
	  
	  //Config PHY's LEDs
 	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2,REALTEK_LED_CFG1_REG,REALTEK_LED_CFG1); 	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2,REALTEK_LED_CFG2_REG,REALTEK_LED_CFG2);
	  
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 31, 0x0000);
	  
	  //switch reg page
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 31, 0x0007);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 30, 0x00a0);
	  //enable SSC
	  mii_mgr_read(MAC_TO_GIGAPHY_MODE_ADDR2, 26, &phy_val);
	  phy_val &= ~(1<<2);
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 26, phy_val);
	  //switch page back
	  mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 31, 0x0000);
	  
	  //FIXME:RTL8211E LED_CONF conflict with Internal EPHY,We must rest the ephy for default reg value.
 	  // reset phy
 	  RALINK_REG(RT2880_RSTCTRL_REG) |= (1<<24);
	  udelay(10);
 	  RALINK_REG(RT2880_RSTCTRL_REG) &= ~(1<<24);
...
{
	  RALINK_REG(RT2880_SYSCFG1_REG) |= (0x3<<14); ////GE2_MODE=EPHY Mode
}
#endif 
	...
	*(unsigned long *)(0xb0000060) &= ~(3 << 7); //set MDIO to Normal mode
	...
	if(isRealtekGigaPHY(2)) 
	{
	    printf("GE2:Found Realtek RTL8211E!\n");
	    mii_mgr_read(MAC_TO_GIGAPHY_MODE_ADDR2, 0x10, &phy_val);
	    phy_val |= 1<<4; //disable CLK125
	    mii_mgr_write(MAC_TO_GIGAPHY_MODE_ADDR2, 0x10, phy_val);
	}
#endif
