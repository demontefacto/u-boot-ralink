...
static void flash_get_offsets(ulong base, flash_info_t *info)
{
	...
	if ((info->flash_id & FLASH_VENDMASK) == FLASH_MAN_MX
		&& ((info->flash_id & FLASH_TYPEMASK) == MX_ID_LV320BT ||
		    (info->flash_id & FLASH_TYPEMASK) == MX_ID_29LV640ET ||
...
/*
 *   Lintel added on 2012.03.31
 * 
 *   Fix for SPANAION S29GLXXX Erase Block
 * 
 */
	else if ((info->flash_id & FLASH_VENDMASK) == FLASH_MAN_AMD
		&& ((info->flash_id & FLASH_TYPEMASK) == MX_ID_29GL128EHT)) {
		...
		sect_size = info->block_size * (sizeof(FPW)/2); /* auto select*/
		
		for (i = 0; i < info->sector_count; i++) {
		    info->start[i] = base + ( i * sect_size);
		}
		printf("S29GLXXX:set sect_size =0x%x\n",sect_size);
	}
	else /*  if ((info->flash_id & FLASH_VENDMASK) == FLASH_MAN_AMD
...
//     printf("\n The Flash ID2 =%08X \n",addr[0x0e]);
	...
	if (info->flash_id != FLASH_UNKNOWN)
		...
		case (FPW)MX_ID_29LV640DB:
			...
			info->size = 0x00800000 * (sizeof(FPW)/2);
			break;
		case (FPW)MX_ID_29LV640ET:
			info->flash_id += MX_ID_29LV640ET;
			info->sector_count = 135;
			info->size = 0x00800000 * (sizeof(FPW)/2);
 			break;
		...
		case (FPW) MX_ID_29GL128EHT:
		  switch(addr[0x0e])
		  {
			...
			info->sector_count = 128;
			info->block_size = 0x20000;
			info->size = 0x01000000 * (sizeof(FPW)/2);
			printf("\nFLASH:S29GL128P/N, SIZE:%d MBytes SECTOR:%d \n",(info->size)/(1024*1024),info->sector_count);
			break;
			...
			info->block_size = 0x20000;
			info->size = 0x01000000 * (sizeof(FPW)/2);
			printf("\nFLASH:S29GL256P/N, SIZE:%d MBytes SECTOR:%d \n",(info->size)/(1024*1024),info->sector_count);
			break;
			
		    case (FPW)0x0000220c:
			info->flash_id +=  MX_ID_29GL128EHT;
			info->sector_count = 128;
			info->block_size = 0x10000;
			info->size = 0x08000000 * (sizeof(FPW)/2);
			printf("\nFLASH:S29GL064N, SIZE:%d MBytes SECTOR:%d \n",(info->size)/(1024*1024),info->sector_count);
			break;
			...
			info->block_size = 0x10000;
			info->size = 0x04000000 * (sizeof(FPW)/2);
			printf("\nFLASH:S29GL032N, SIZE:%d MBytes SECTOR:%d \n",(info->size)/(1024*1024),info->sector_count);
			break;
			
		    default:
			printf("\nThis S29GLXXX FLASH Not Supported!! Try Default!");
			info->flash_id +=  MX_ID_29GL128EHT;
			info->sector_count = 128;
			info->block_size = 0x10000;
			info->size = 0x08000000 * (sizeof(FPW)/2);
			printf("FLASH:S29GLXXX, SIZE:%d MBytes SECTOR:%d \n",(info->size)/(1024*1024),info->sector_count);
			break;			    
	...
	/* Put FLASH back in read mode */
	flash_reset(info);
//	printf("FLASH::%d MBytes  blocks:%d \n",(info->size)/(1024*1024),info->sector_count);
	return (info->size);
}
