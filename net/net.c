...
NetSendHttpd(void)
{
	...
	(void) eth_send((uchar*)NetTxPacket, uip_len);
}
...
NetLoopHttpd(void)
{
	...
	eth_initialize(gd->bd); /*initialize network*/
...
//	ip[0]=0xA8C0;
//	ip[1]=0x0101; 
	/*ip[0] = ((bd->bi_ip_addr & 0x000000ff) << 8) | ((bd->bi_ip_addr & 0x0000ff00) >> 8);
        ip[1] = ((bd->bi_ip_addr & 0x00ff0000) >> 8) | ((bd->bi_ip_addr & 0xff000000)>>24);*/
	show_httpd_logo();
	printf("uHttpd Server started.\n");

    /*Host IP:192.168.1.1*/
#if 0
//	uip_ipaddr(ip, 192,168,1,1);
	uip_ipaddr(ip,(bd->bi_ip_addr & 0x000000ff), 
		   (bd->bi_ip_addr & 0x0000ff00) >> 8, 
		   (bd->bi_ip_addr & 0x00ff0000) >> 16, 
		   (bd->bi_ip_addr & 0xff000000) >> 24);
#endif
	NetCopyIP(ip, &bd->bi_ip_addr);
	uip_sethostaddr(ip);
	uip_gethostaddr(ip);
...
//	uip_ipaddr(ip, 255,255,255,0);
    ...
    /* Host default route: 0.0.0.0 */
	uip_ipaddr(ip, 0,0,0,0);
	uip_setdraddr(ip);
	uip_getdraddr(ip);
        printf("Default Router:%ld.%ld.%ld.%ld\n",
                ip[0] & 0x00ff,
                ip[0] >> 8,
                ip[1] & 0x00ff,
                ip[1] >> 8);
	...
	for (;;) {
		...
		do_firmware_upgrade((uchar *)CFG_LOAD_ADDR,NetBootFileXferSize,image_type);
               // *(unsigned long*)(0xb0000620)=(*(unsigned long*)(0xb0000620)) & (~(1<<9)) | ((1<<8) | (1<<13));
